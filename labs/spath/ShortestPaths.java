...
public class ShortestPaths {
    ...
    public void run() {
    	...
    	// FIXME
    	//
    	VertexAndDist leastVertex ; 
    	Vertex parent = null;
    	int priorPathWt;
    	while(!pq.isEmpty()) {
    		leastVertex = pq.peek(1);
    		Decreaser<VertexAndDist> vid = handles.get(leastVertex.vertex);
    		priorPathWt = vid.getValue().distance;
    		for (Edge e : leastVertex.vertex.edgesFrom()) {
    			
    			int newDist = priorPathWt + this.weights.get(e);
    			int oldDist = handles.get(e.to).getValue().distance;
    			if(newDist < oldDist) {
    				handles.get(e.to).decrease(new VertexAndDist(e.to, newDist));
    ...
    public LinkedList<Edge> returnPath(Vertex endVertex) {
	   ...
	   path.addAll(returnPath(parentEdges.get(endVertex).from));
