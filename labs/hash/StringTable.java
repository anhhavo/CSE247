...
public class StringTable {
    ...
    {  
    	...
    	int bucket = toIndex(stringToHashCode(r.key));
    ...
    public Record find(String key) 
    {
    	...
    	int bucket = toIndex(stringToHashCode(key));
    	
    	// Now search the bucket for the key
    	for(Record r : buckets[bucket]) {
    		if(r.key.equals(key)) {
    			return r;
    		}
    	}
    ...
    public void remove(String key) 
    {
    	...
    	Record r = find(key);
    	if(r != null) {
    		buckets[bucket].remove(r);
        	size--;
    	}
    	// If the code reaches here, that means the key is not found
    }
